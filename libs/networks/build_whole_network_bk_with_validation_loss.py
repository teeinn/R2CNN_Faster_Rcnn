...
class DetectionNetwork(object):
    ...
    def __init__(self, base_network_name, is_training, validation_loss=False):
    ...
    def build_fastrcnn(self, feature_to_cropped, rois, img_shape):
        ...
        with tf.variable_scope('Fast-RCNN'):
            ...
            if self.base_network_name.startswith('resnet'):
                fc_flatten, logits = resnet.restnet_head(input=pooled_features,
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch):
        ...
        if self.is_training or self.validation_loss:
        ...
        with tf.variable_scope('build_rpn',
            ...
            rpn_conv3x3 = slim.conv2d(
                ...
                trainable=self.is_training or self.validation_loss, weights_initializer=cfgs.INITIALIZER,
            ...
            rpn_box_pred = slim.conv2d(rpn_conv3x3, self.num_anchors_per_location*4, [1, 1], stride=1,
                                       trainable=self.is_training or self.validation_loss, weights_initializer=cfgs.BBOX_INITIALIZER,
                                       activation_fn=None,
                                       scope='rpn_bbox_pred')
        ...
        with tf.variable_scope('postprocess_RPN'):
            ...
            rois, roi_scores = postprocess_rpn_proposals(rpn_bbox_pred=rpn_box_pred,
                                                         ...
                                                         img_shape=img_shape,
                                                         anchors=anchors,
                                                         is_training=self.is_training or self.validation_loss)
            # rois shape [-1, 4]
            # +++++++++++++++++++++++++++++++++++++add img smry+++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        # cls_score_r = tf.divide(cls_score_r, temp_var)
        ...
        if not self.is_training:
            ...
            #temp scaling
            # return final_boxes_h, final_scores_h, final_category_h, final_boxes_r, final_scores_r, final_category_r, cls_score_r, rois, bbox_pred_r, img_shape
        else:
            '''
