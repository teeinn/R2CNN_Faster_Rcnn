...
EVAL_INTERVAL = 100 #60 seconds, for test
...
def eval_with_plac(img_dir, det_net, num_imgs, image_ext, draw_imgs, test_annotation_path):
    ...
    det_boxes_h, det_scores_h, det_category_h, \
    det_boxes_r, det_scores_r, det_category_r, loss_dict = det_net.build_whole_detection_network(
        input_img_batch=img_batch,
        gtboxes_h_batch=gtboxes_and_label, gtboxes_r_batch=gtboxes_and_label_AreaRectangle)
        ...
        if model_path == None:
            ...
            with tf.Session(config=config) as sess:
                ...
                for i, a_img_name in enumerate(imgs):
                    ...
                    det_boxes_r_, det_scores_r_, det_category_r_, validation_total_loss = \
                        sess.run(
                            [img_batch, det_boxes_h, det_scores_h, det_category_h,
                             det_boxes_r, det_scores_r, det_category_r, loss_dict],
                ...
                mAP_h, mRecall_h, mPrecision_h, Ap_dict_h, Recall_dict_h, Precision_dict_h = voc_eval.voc_evaluate_detections(
                    all_boxes=all_boxes_h,
                    test_imgid_list=real_test_imgname_list,
                    test_annotation_path=test_annotation_path)

                mAP_r, mRecall_r, mPrecision_r, Ap_dict_r, Recall_dict_r, Precision_dict_r = voc_eval_r.voc_evaluate_detections(
                    all_boxes=all_boxes_r,
                    test_imgid_list=real_test_imgname_list,
                    test_annotation_path=test_annotation_path)

                total_ls = {"mAP_h": mAP_h, "mAP_r": mAP_r, "mRecall_h": mRecall_h, "mRecall_r": mRecall_r,
                            "mPrecision_h": mPrecision_h, "mPrecision_r": mPrecision_r}
                ...
                rpn_location_loss = validation_total_loss['rpn_loc_loss']
                rpn_cls_loss = validation_total_loss['rpn_cls_loss']
                rpn_total_loss = rpn_location_loss + rpn_cls_loss
                fastrcnn_cls_loss_h = validation_total_loss['fastrcnn_cls_loss_h']
                fastrcnn_loc_loss_h = validation_total_loss['fastrcnn_loc_loss_h']
                fastrcnn_cls_loss_r = validation_total_loss['fastrcnn_cls_loss_r']
                fastrcnn_loc_loss_r = validation_total_loss['fastrcnn_loc_loss_r']
                ...
                new_summary = tf.Summary()
                new_summary.value.add(tag='VALIDATION_LOSS/total_loss', simple_value=total_loss)
                summary_writer.add_summary(new_summary, global_step)
...
def eval(num_imgs, img_dir, image_ext, test_annotation_path):
    ...
    faster_rcnn = build_whole_network.DetectionNetwork(base_network_name=cfgs.NET_NAME,
                                                       is_training=False, validation_loss=True)
