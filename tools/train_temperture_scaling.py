...
def train():
    ...
    with slim.arg_scope([slim.conv2d, slim.conv2d_in_plane,
        ...
        final_boxes_r, final_scores_r, final_category_r, loss_dict, \
            rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels, bbox_pred_h, bbox_targets_h, cls_score_h, bbox_pred_r, bbox_targets_r, cls_score_r, labels\
    ...
    # lr = tf.train.piecewise_constant(global_step,
    #                                  boundaries=[np.int64(cfgs.DECAY_STEP[0]), np.int64(cfgs.DECAY_STEP[1]), np.int64(cfgs.DECAY_STEP[2]), np.int64(cfgs.DECAY_STEP[3])],
    #                                  values=[cfgs.LR, cfgs.LR / 10., cfgs.LR / 100., cfgs.LR / 10., cfgs.LR / 100.])
    ...
    cls_score_r_ = tf.divide(cls_score_r, temp_var)
    loss_dict_ = faster_rcnn.build_loss(rpn_box_pred=rpn_box_pred,
                                ...
                                cls_score_r=cls_score_r_,
                                labels=labels)
    _fastrcnn_cls_loss_r = loss_dict_['fastrcnn_cls_loss_r']
    optim = tf.contrib.opt.ScipyOptimizerInterface(_fastrcnn_cls_loss_r, options={'maxiter': 10000})
    ...
    with tf.Session(config=config) as sess:
        ...
        optim.minimize(sess)
        nll_loss = sess.run(_fastrcnn_cls_loss_r)
        ...
        #         _, global_stepnp, _fastrcnn_cls_loss_r__ = sess.run([train_op, global_step, _fastrcnn_cls_loss_r])
        #
        #     else:
        #         if step % cfgs.SHOW_TRAIN_INFO_INTE == 0 and step % cfgs.SMRY_ITER != 0:
        #             start = time.time()
        ...
        #             _fast_rcnn_total_loss, _total_loss, _, _temp_var, _fastrcnn_cls_loss_r_test = \
        ...
        #                           fastrcnn_total_loss, total_loss, train_op, temp_var, _fastrcnn_cls_loss_r])
        #
        #             end = time.time()
        #             print(""" {}: step{}    image_name:{} |\t
        #                                             temp_var:{} |\t
        #                                             fastrcnn_cls_loss_r_test:{} |\t
        ...
        #                   .format(training_time, _global_step, str(_img_name_batch[0]), _temp_var, _fastrcnn_cls_loss_r_test, _rpn_location_loss,
