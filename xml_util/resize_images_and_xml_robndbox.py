...
def build_xml_arch(parsed_xml, new_height, new_width, origin_height, origin_width, resized_height, resized_width, resize_ratio):
    label = ''
    cx = 0
    cy = 0
    width = 0
        ...
        if not obj.find('robndbox') is None:
            if resize_ratio > 1:
                ...
                try:
                    ...
                    x0 = str(float(obj.find('./robndbox/x0').text) * resize_ratio)
                    x1 = str(float(obj.find('./robndbox/x1').text) * resize_ratio)
                    x2 = str(float(obj.find('./robndbox/x2').text) * resize_ratio)
                    x3 = str(float(obj.find('./robndbox/x3').text) * resize_ratio)
                    y0 = str(float(obj.find('./robndbox/y0').text) * resize_ratio)
                    y1 = str(float(obj.find('./robndbox/y1').text) * resize_ratio)
                    y2 = str(float(obj.find('./robndbox/y2').text) * resize_ratio)
                    y3 = str(float(obj.find('./robndbox/y3').text) * resize_ratio)
            ...
            #
            # if min(float(x0), float(x1), float(x2), float(x3)) < 0 or min(float(y0), float(y1), float(y2), float(y3)) < 0 or max(float(x0), float(x1), float(x2), float(x3)) >= resized_width or max(float(y0), float(y1), float(y2), float(y3)) >= resized_height:
            #     continue
...
def make_noise(resized_image, resized_width, resized_height, origin_height, origin_width):
    noise_img = (np.uint8(np.random.rand(origin_height, origin_width, 3) * 255))
    # noise_img = np.zeros([origin_width, origin_height, 3])
...
def xml_parser(root, file, new_height, new_width, origin_height, origin_width, resized_height, resized_width, resize_ratio):
    ...
    new_xml_tree = build_xml_arch(xml_parse, new_height, new_width, origin_height, origin_width, resized_height, resized_width, resize_ratio)
        ...
        for root, dirs, files in os.walk(src_dir):
            ...
            for file in files:
                ...
                try:
                    ...
                    new_xml_tree = xml_parser(root, file, new_height, new_width, origin_height, origin_width, resized_height, resized_width, resize_ratio)
...
def walk_around_xml_files_(resize_ratio, src_anno_dir_list, dst_anno_dir_list, src_img_dir_list, dst_img_dir_list):

    if not os.path.exists(dst_img_dir_list[0]):
        os.makedirs(dst_img_dir_list[0])
    err_file_cnt = 0
    for src_dir in src_img_dir_list:
        for root, dirs, files in os.walk(src_dir):
           for file in files:
                current_img_file = os.path.join(root, file)
                resize_tag = '_' + str(resize_ratio)
                dst_img_file = os.path.join(dst_img_dir_list[0], file)
...
if __name__ == "__main__":
    ...
    src_dir = "/media/qisens/2tb1/python_projects/inference_pr/tfrecord-viewer/new_dataset_overfitting_test13_riverarea_area123"
    separated_dir = "/media/qisens/2tb1/python_projects/inference_pr/tfrecord-viewer/new_dataset_overfitting_test13_riverarea_area123/train/resized_0.8"
